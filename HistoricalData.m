...
for i = 2:dataMax
    ...
    payload = data{i,iPayload};
    endurance = data{i,iEndurance};
    if ~(isnan(mtow) || isnan(payload) || isnan(endurance))
        if endurance < 30
            h1 = plot(mtow, payload, 'k.');
        elseif endurance < 50
            h2 = plot(mtow, payload, 'r.');
        else
            h3 = plot(mtow,payload, 'b.');
        end
        allMtow = [allMtow mtow];
        allPayload = [allPayload payload];
        hold on;
...
y = p(1).*x + p(2);
plot(x,y,'k:');
h4 = plot(designMtow, designPayload, 'bx');
xlim([0 50000])
xlabel('MTOW (kg)');
ylabel('Payload (kg)');
set(gca,'FontSize',20)
set(gcf,'Position',[00 400 670 520])
legend([h1,h2,h3,h4],'t < 30hr','30hr < t < 50hr','t > 50hr','Our Solution', 'Location','northwest');
grid on
