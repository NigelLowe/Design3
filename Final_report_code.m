...
mission_PL           = [3000,500,3000,500];
mission_weight       = [18.5e3,18.5e3,22.5e3,22.5e3];
endurance_target_mat = [24,32,50,62];
...
for mission_index = missionIndecies
        ...
        cruising_alt_mat = convlength(linspace(40e3,30e3,5),'ft','m');
 ...
 %create ALT vs Vel Contour (with endurance)
    if ~exist('plotOtherGraphs','var')
         ...
         hold on
           ...
           plot(stall_speed*1.9438,cruising_alt_mat*3.2808,'LineWidth',4,'linestyle','--','color',myred)
            ...
            plot(mach_limit_speed*1.9438,cruising_alt_mat*3.2808,'LineWidth',4,'linestyle','--','color',myblue)
